 ...
 * Navbar.
 *
 * Standalone to this vertical: no imports cross from the /attorney, /doctor or
 * /restaurant modules, so each landing page stays independently editable.
 *
 * Structure is the /restaurant navbar as requested — same 1420px bar, right-hand
 * menu+CTA cluster, sticky-past-100px behaviour and mobile drawer. Only the palette
 * differs.
 *
 * The logo is a static import from `src/assets/`, which is where it was supplied, so
 * next/image reads its intrinsic 193x41 size and no width/height is declared by hand.
 */
...
const DRAWER_ID = "plumber-mobile-drawer";
...
const PlumberNavbar = () => {
  ...
  return (
    <>
      ...
      <header
        ref={headerRef}
        // The bar is already an opaque #FCFFEA, so nothing needs to change for
        // legibility once content scrolls under it — only the shadow is added to lift
        // it off the page.
        className={
          isPinned
            ? "animate-nav-slide-down fixed inset-x-0 top-0 z-50 w-full bg-[#FCFFEA] shadow-[0_4px_20px_rgba(15,26,26,0.08)]"
            : "relative w-full bg-[#FCFFEA]"
        }
      >
            ...
            */}
            <div className="flex items-center gap-4 lg:gap-10 xl:gap-[75px]">
              ...
              <nav className="hidden items-center gap-5 xl:gap-[31px] lg:flex">
                ...
                {navLinks.map((link) => (
                  <a
                    ...
                    href={link.href}
                    style={navTypography}
                    className="whitespace-nowrap text-[16px] text-[#0F1A1A] transition-colors duration-200 hover:text-[#8FA800]"
                  >
                    {link.label}
                  </a>
                ))}
              ...
              <Link
                href="/contact-us"
                style={navTypography}
                className="hidden items-center gap-2 rounded-[10px] bg-[#B6D500] px-4 xl:px-[30px] py-3 xl:py-4 whitespace-nowrap text-[#101311] text-[14px] xl:text-[16px] transition-colors duration-200 hover:bg-[#a2bf00] lg:inline-flex"
              >
              ...
              <button
                ...
                className="text-[#0F1A1A] transition-colors duration-200 hover:text-[#8FA800] lg:hidden"
      ...
      <div
        id={DRAWER_ID}
        inert={!mobileOpen}
        className={`fixed top-0 left-0 z-50 flex h-dvh w-80 max-w-[85%] flex-col bg-[#FCFFEA] transition-transform duration-300 ease-out lg:hidden ${
      ...
      >
        <div className="flex shrink-0 items-center justify-between border-b border-[#0F1A1A]/10 p-5">
          ...
          <button
            ...
            className="text-[#0F1A1A] transition-colors duration-200 hover:text-[#8FA800]"
          >
            <X className="size-7" />
          </button>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto">
            ...
            {navLinks.map((link) => (
              <li key={link.label}>
                ...
                <a
                  ...
                  onClick={() => setMobileOpen(false)}
                  style={navTypography}
                  className="block border-b border-[#0F1A1A]/10 px-6 py-4 text-[#0F1A1A] transition-colors duration-200 hover:bg-white/40 hover:text-[#8FA800]"
                >
                  {link.label}
                </a>
              </li>
          ...
          <div className="p-6">
            <Link
              ...
              onClick={() => setMobileOpen(false)}
              style={navTypography}
              className="flex items-center justify-center gap-2 rounded-[10px] bg-[#B6D500] px-3.5 sm:px-6 py-4 text-center text-[#101311] transition-colors duration-200 hover:bg-[#a2bf00]"
            >
              Book a Free Consultation
              <ArrowUpRight className="size-4" aria-hidden="true" />
            </Link>
