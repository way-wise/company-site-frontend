...
import logo from "@/assets/images/plumber/footer_logo.webp";

/**
 * Footer.
 *
 * Same structure as the /restaurant footer — brand column with logo, blurb and socials,
 * then link columns and a bottom bar — with two differences from that page: there is no
 * "Start a Conversation" card, and the third column is contact details plus a CTA rather
 * than another link list.
 *
 * Standalone: no imports cross from the other verticals, so each stays independently
 * editable.
 *
 * The logo is a static import from `src/assets/`, so next/image reads its intrinsic
 * 190x40 size and no width/height is declared by hand.
 *
...
/** Body copy and link text. Figma spec: 18px / 20px, #F7F8F3. */
...
/** The brand blurb wraps to two lines, so it gets real leading rather than the 20px. */
const blurbTypography = {
  fontFamily: "var(--font-plus-jakarta-sans), sans-serif",
  fontSize: "18px",
  lineHeight: "28px",
  letterSpacing: "0",
...
// Figma spec: Plus Jakarta Sans Bold 18px / 16px, 0.3px letter-spacing, uppercase,
// #B6D500.
...
// Figma spec: Plus Jakarta Sans SemiBold 16px / 130%, zero letter-spacing, centered,
// #101311.
...
// so every solution entry points at the /services index rather than guessing a slug.
const solutionLinks = [
  { label: "Trade Websites", href: "/services" },
  { label: "Client Portals", href: "/services" },
  { label: "Project Dashboards", href: "/services" },
  { label: "CRM & Lead Tools", href: "/services" },
  { label: "Mobile Apps", href: "/services" },
  { label: "Custom Software", href: "/services" },
];

const industryLinks = [
  { label: "Plumbing", href: "/plumber" },
  { label: "Electrical", href: "#" },
  { label: "HVAC & Heating", href: "#" },
  { label: "Construction", href: "#" },
  { label: "Repair & Handyman", href: "#" },
  { label: "Roofing", href: "#" },
];
...
// Inline SVG paths rather than lucide's Facebook/Linkedin/Youtube components, which are
// deprecated in lucide-react (brand marks are being removed from the set). The shared
// site Footer hand-rolls its social SVGs for the same reason.
const socialPaths = {
  linkedin:
...
const LinkColumn = ({
  ...
  <div>
    ...
    <h2 className="mb-6 text-[#B6D500] uppercase" style={headingTypography}>
      {heading}
    </h2>
      ...
      {links.map((link) => (
        <li key={link.label}>
          <Link
            ...
            className="text-[#F7F8F3] transition-colors duration-200 hover:text-[#B6D500]"
          >
            {link.label}
          </Link>
        </li>
...
const PlumberFooter = () => {
  return (
    <footer id="contact-us" className="scroll-mt-[110px] bg-[#101311]">
      {/* 1420px, matching the navbar. Padding on the outer element, cap on the inner
          one, so content measures its stated width at wide viewports rather than that
          width minus the gutters. */}
      <div className="w-full px-4">
        <div className="mx-auto w-full max-w-[1420px] pt-10 pb-8 lg:pt-16">
          {/* The three right-hand columns are `auto`, so each is only as wide as its own
              longest entry, while the brand column takes `1fr` and absorbs the slack. */}
          <div className="grid grid-cols-1 gap-12 md:grid-cols-2 lg:grid-cols-[1fr_auto_auto_auto] lg:gap-x-14">
            <div className="max-w-[560px]">
              ...
              <p className="mt-7 text-[#F7F8F3]" style={blurbTypography}>
                Digital growth and operations partner for trade and construction
                businesses. From brand identity to full operations management.
              </p>
                ...
                {socials.map(({ label, href, path }) => (
                  <li key={label}>
                    <a
                      ...
                      aria-label={label}
                      className="flex size-9 items-center justify-center rounded-md border border-white/15 text-[#F7F8F3] transition-colors duration-200 hover:border-[#B6D500] hover:text-[#B6D500]"
                    >
                      {/* currentColor so the mark follows the link's hover colour — a
                          hardcoded stroke would stay put while the border moved. */}
            ...
            <LinkColumn heading="Industries" links={industryLinks} />

            {/* Contact column: details rather than a link list, then the CTA. */}
            <div>
              <h2
                className="mb-6 text-[#B6D500] uppercase"
              ...
              <ul className="flex flex-col gap-4">
                <li>
                  <a
                    href="mailto:hello@waywisetech.com"
                    style={bodyTypography}
                    className="text-[#F7F8F3] transition-colors duration-200 hover:text-[#B6D500]"
                  >
                    hello@waywisetech.com
                  </a>
                </li>
                <li>
                  {/* Vanity number: the tel: href carries the dialable digits while the
                      visible label keeps the lettering. */}
                  <a
                    href="tel:+18009299473"
                    style={bodyTypography}
                    className="text-[#F7F8F3] transition-colors duration-200 hover:text-[#B6D500]"
                  >
                    1-800-WAY-WISE
                ...
                <li className="text-[#F7F8F3]" style={bodyTypography}>
                  United States
                </li>
              </ul>
              ...
              <Link
                ...
                className="mt-6 inline-block rounded-md bg-[#B6D500] px-[25px] py-2.5 text-center whitespace-nowrap text-[#101311] transition-colors duration-200 hover:bg-[#a2bf00]"
              >
                Get Free Assessment
              </Link>
            </div>
          ...
          {/* Bottom bar */}
          <div className="mt-14 flex flex-col items-center gap-6 border-t border-[#757575] pt-6 lg:flex-row lg:justify-between lg:gap-0">
            <p className="text-[#F7F8F3]" style={bodyTypography}>
