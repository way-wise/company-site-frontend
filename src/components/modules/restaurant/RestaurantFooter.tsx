 ...
 * Footer.
 *
 * Same structure as the /doctor footer, as requested — brand column with the logo,
 * blurb, socials and a "Start a Conversation" card, then three link columns and a
 * bottom bar. Only the palette differs: dark ground with light copy here, rather than
 * that page's light theme.
 *
 * Standalone: no imports cross from the /attorney or /doctor modules, so each vertical
 * stays independently editable.
 *
 * The logo lives in `public/` rather than `src/assets/`, which is where it was supplied,
 * so it is referenced by path and needs its intrinsic size declared.
 *
 ...
 */
const LOGO = "/images/restaurant/white_logo.webp";
...
const industryLinks = [
  { label: "Healthcare & Medical", href: "/doctor" },
...
const LinkColumn = ({
  ...
  <div>
    ...
    <h2 className="mb-6 text-white" style={headingTypography}>
      {heading}
    </h2>
      ...
      {links.map((link) => (
        <li key={link.label}>
          <Link
            ...
            className="text-[#EDE6E1] transition-colors duration-200 hover:text-[#E94222]"
          >
            {link.label}
          </Link>
        </li>
    ...
    <Link
      ...
      className="mt-6 inline-block text-white transition-colors duration-200 hover:text-[#E94222]"
    >
      {cta.label}
    </Link>
  </div>
  ...
  return (
    <footer id="contact-us" className="bg-[#16110E]">
      ...
      <div className="w-full px-4">
        <div className="mx-auto w-full max-w-[1320px] pt-10 pb-8 lg:pt-16">
          ...
          <div className="grid grid-cols-1 gap-12 md:grid-cols-[1fr_auto_auto_auto] lg:gap-x-14">
            <div className="max-w-[560px]">
              ...
              <p className="mt-7 text-[#EDE6E1]" style={bodyTypography}>
                A global technology company designing and building digital
                products for ambitious businesses in the USA, UAE, and
                worldwide.
              </p>
                ...
                {socials.map(({ label, href, path }) => (
                  <li key={label}>
                    <a
                      ...
                      aria-label={label}
                      className="flex size-9 items-center justify-center rounded-md border border-white/15 text-[#EDE6E1] transition-colors duration-200 hover:border-[#E94222] hover:text-[#E94222]"
                    >
              ...
              <div className="mt-10 rounded-lg bg-[#FFE0D1] p-4">
                <p className="mb-3.5 text-[#17120F]" style={bodyTypography}>
                  Start a Conversation
                </p>
                <Link
                  href="/contact-us"
                  style={bodyTypography}
                  className="block rounded-md bg-[#E94222] py-3 text-center font-semibold text-white transition-colors duration-200 hover:bg-[#cf3517]"
                >
                  Get in Touch
                </Link>
              </div>
          ...
          {/* Bottom bar */}
          <div className="mt-14 flex flex-col items-center gap-6 border-t border-white/10 pt-6 lg:flex-row lg:justify-between lg:gap-0">
            <p className="text-[#EDE6E1]" style={bodyTypography}>
