    ...
    },
    {
      ...
      cell: ({ row }: { row: { original: NewLiveProject } }) => {
        ...
        const DuePaymentDisplay = () => {
          ...
          return (
            <span className={`text-sm font-medium ${duePayment > 0 ? "text-red-600" : "text-green-600"}`}>
              {duePayment > 0 
                ? `$${duePayment.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` 
                : "$0.00"}
            </span>
          );
        };
    ...
    <div className="space-y-4">
      ...
      <TimesheetModal
        ...
        onProjectUpdate={async () => {
          ...
          if (result.data?.data && selectedProject) {
